...
export const AdminMessages = () => {
    ...
    async function submitResponseToQuestion(id: number, response: String) {
        ...
        if (authState && authState?.isAuthenticated && id !== null && response !== '') {
            const messageAdminRquestModel: AdminMessageRequest = new AdminMessageRequest(id, response);
            const requestOptions = {
                method: 'PUT',
                headers: {
                    Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
                ...
                body: JSON.stringify(messageAdminRquestModel)
            };

            const messageAdminRquestModelResponse = await fetch(url, requestOptions);
            if(!messageAdminRquestModelResponse.ok){
